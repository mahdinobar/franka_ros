...
KalmanFilter::KalmanFilter(Eigen::MatrixXd Ainput, Eigen::MatrixXd Binput, Eigen::MatrixXd Cinput,
                           Eigen::MatrixXd Qinput, Eigen::MatrixXd Rinput, Eigen::MatrixXd P0input,
                           Eigen::MatrixXd x0input, unsigned int maxSimulationSamples) {
...
//  TODO
  X_prediction_ahead.resize(n, 2);
  X_prediction_ahead.col(0) = x0;
...
void KalmanFilter::predictEstimate(Eigen::MatrixXd u) {
  // keep in mind that initially, k_measured=0
  estimatesApriori.col(0) = A * estimatesAposteriori.col(0) + B * u;
  covarianceApriori(all, seq(0, n - 1)) =
      A * covarianceAposteriori(all, seq(0, n - 1)) * (A.transpose()) + Q;
}

void KalmanFilter::prediction_aheads(Eigen::MatrixXd u) {
  X_prediction_ahead.col(1) = A * X_prediction_ahead.col(0) + B * u;
}
...
void KalmanFilter::updateEstimate(double measurement_x, double measurement_y, double measurement_z) {
  cout << "k_measured=" << k_measured << endl;
  ...
  Eigen::MatrixXd Sk;
  Sk.resize(r, r);
  Sk = R + C * covarianceApriori(all, seq(0, n - 1)) * (C.transpose());
  Sk = Sk.inverse();
  // gain matrices
  gainMatrices(all, seq(0, r - 1)) =
      covarianceApriori(all, seq(0, n - 1)) * (C.transpose()) * Sk;
  ...
  Y(2) = measurement_z;
  //    cout << Y << endl;
  errors.col(0) = Y - C * estimatesApriori.col(0);
  // compute the a posteriori estimate, remember that for k_measured=0, the corresponding column is x0 - initial guess
  estimatesAposteriori.col(0) =
      estimatesApriori.col(0) +
      gainMatrices(all, seq(0, r - 1)) * errors.col(0);

  Eigen::MatrixXd In;
  In = Eigen::MatrixXd::Identity(n, n);
  Eigen::MatrixXd IminusKC;
  IminusKC.resize(n, n);
  IminusKC = In - gainMatrices(all, seq(0, r - 1)) * C;  // I-KC

  // update the a posteriori covariance matrix
  covarianceAposteriori(all, seq(0, n - 1))
      = IminusKC * covarianceApriori(all, seq(0, n - 1)) * (IminusKC.transpose())
        + gainMatrices(all, seq(0, r - 1)) * R *
              (gainMatrices(all, seq(0, r - 1)).transpose());
}
...
// Eigen::MatrixXd is an Eigen typdef for Eigen::Matrix<double, Dynamic, Dynamic>

Eigen::MatrixXd KalmanFilter::openData(string fileToOpen) {
  ...
  while (getline(matrixDataFile,
                 matrixRowString)) // here we read a row by row of matrixDataFile and store every line into the string variable matrixRowString
  {
                   ...
                   ',')) // here we read pieces of the stream matrixRowStringStream until every comma, and store the resulting character into the matrixEntry
    {
      matrixEntries.push_back(
          stod(matrixEntry));   //here we convert the string to double and fill in the row vector storing all the matrix entries
    }
  ...
  return Map<Eigen::Matrix<double, Dynamic, Dynamic, RowMajor>>(matrixEntries.data(),
